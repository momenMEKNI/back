...
void freeJoueur(Joueur *j) {
    if (!j) return;
    if (j->idleTexture) SDL_DestroyTexture(j->idleTexture);
    for (int i=0;i<MAX_FRAMES;i++) {
        if (j->walkFrames[i]) SDL_DestroyTexture(j->walkFrames[i]);
        if (j->runFrames[i]) SDL_DestroyTexture(j->runFrames[i]);
        if (j->jumpFrames[i]) SDL_DestroyTexture(j->jumpFrames[i]);
        if (j->attackFrames[i]) SDL_DestroyTexture(j->attackFrames[i]);
        if (j->deadFrames[i]) SDL_DestroyTexture(j->deadFrames[i]);
    }
}
